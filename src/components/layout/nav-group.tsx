...
function SidebarMenuLink({ item, href }: { item: NavLink; href: string }) {
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        asChild
        isActive={checkIsActive(href, { ...item, url: resolvedUrl })}
        tooltip={item.title}
      >
        <Link to={resolvedUrl} onClick={() => setOpenMobile(false)}>
          {item.icon && <item.icon />}
            ...
            {item.items.map((subItem) => {
              ...
              return (
                <SidebarMenuSubItem key={subItem.title}>
                  <SidebarMenuSubButton
                    asChild
                    isActive={checkIsActive(href, { ...subItem, url: resolvedSubUrl })}
                  >
                    <Link to={resolvedSubUrl} onClick={() => setOpenMobile(false)}>
          ...
          {item.items.map((sub) => {
            ...
            return (
              <DropdownMenuItem key={`${sub.title}-${resolvedSubUrl}`} asChild>
                <Link
                  to={resolvedSubUrl}
                  className={`${checkIsActive(href, { ...sub, url: resolvedSubUrl }) ? 'bg-secondary' : ''}`}
                >
                  {sub.icon && <sub.icon />}
